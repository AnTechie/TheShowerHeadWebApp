...
static bool buttonUpState = false;
    ...
    size_t payloadSize, void* userContextCallback)
{
	...
	JSON_Object* buttonOrientation = json_object_dotget_object(desiredProperties, "Orientation");
	if (buttonOrientation != NULL) {
		buttonUpState = (bool)json_object_get_boolean(desiredProperties, "value");
		/*GPIO_SetValue(deviceTwinRipleyLedGpioFd,
			(buttonUpState == true ? GPIO_Value_Low : GPIO_Value_High));*/
		TwinReportBoolState("Orientation", buttonUpState);
	}
cleanup:
...
static void SendOrientationButtonHandler(void)
{
    if (IsButtonPressed(sendOrientationButtonGpioFd, &sendOrientationButtonState)) {
        ...
        SendTelemetry("Orientation", deviceIsUp ? "true" : "false");
    }
}
